...
onMounted(async () => {
  const pathPosition = [
    ...
    [50, -122.9385, 39.50883, 1772],
    [100, -122.93797, 39.5087, 1771]
  ]
  const czml = [
    {
      id: 'document',
    ...
    },
    {
      ...
      // billboard: {
      //   image: Cesium.Cesium3DTileset.fromUrl('./tiles/car1/tileset.json'),
      ...
      position: {
        ...
        cartographicDegrees: pathPosition.flat()
      }
    }
  ]
  ...
  const dataSource = await viewer.dataSources.add(Cesium.CzmlDataSource.load(czml))

  viewer.trackedEntity = dataSource.entities.getById('path') // 按照轨迹追逐视角
